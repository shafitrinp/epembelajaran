...
class C_soal extends CI_Controller {
    ...
    public function index($id_tEssay=null){
        ...
        if($id_tEssay!=null){
        	$query_soal = $this->cbt_soal_model->get_by_kolom_limit('id_tEssay', $id_tEssay, 1);
        	if($query_soal->num_rows()>0){
        		$query_soal = $query_soal->row();
        		$data['id_tEssay'] = $query_soal->id_tEssay;
        		$data['id_jTugas'] = $query_soal->id_jTugas;
        		$data['pertanyaan'] = $query_soal->pertanyaan;
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('id_tEssay', 'Kode Soal','required');
        $this->form_validation->set_rules('id_jTugas', 'Judul Tugas','required');
        $this->form_validation->set_rules('pertanyaan', 'Soal','required');
        
        
        if($this->form_validation->run() == TRUE){
        	$id_soal = $this->input->post('id_tEssay', TRUE);
        	$id_jTugas = $this->input->post('id_jTugas', TRUE);
        	$soal = $this->input->post('pertanyaan', FALSE);
        	// $tipe = $this->input->post('tambah-tipe', TRUE);
        	// $kesulitan = $this->input->post('tambah-kesulitan', TRUE);
        	// $audio = $_FILES['tambah-audio']['name'];

        	$posisi = $this->config->item('upload_path').'/topik_'.$id_tEssay.'';
